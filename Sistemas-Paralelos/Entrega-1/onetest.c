...
int num = 1; 
 for(i=0;i<N;i++){
   for(j=0;j<N;j++){
	setValor(A,i,j,ORDENXFILAS,num);
	setValor(B,i,j,ORDENXFILAS,num+16);
  setValor(C,i,j,ORDENXFILAS,num+32);
	setValor(D,i,j,ORDENXFILAS,num+48);
  num++;
   }
  }   


  void printMatrix(double *m, char *nombre){
    int i,j;
    printf("Matriz %s\n", nombre);
    for(i=0;i<N;i++){
      for(j=0;j<N;j++){
        printf("%f ", getValor(m,i,j,ORDENXFILAS));      
      }
      printf("\n");
    }
  }
