 ...
 //Inicializa las matrices A y B en 1, el resultado sera una matriz con todos sus valores en N
  for(i=0;i<N;i++){
   for(j=0;j<N;j++){
	  ...
	  setValor(AT,i,j,ORDENXCOLUMNAS,1);
	  setValor(BT,i,j,ORDENXCOLUMNAS,1);
 ...
 //Realiza la multiplicacion
  ...
  for(i=0;i<N;i++){
   for(j=0;j<N;j++){
    ...
    for(k=0;k<N;k++){
	    setValor(AB,i,j,ORDENXFILAS, getValor(AB,i,j,ORDENXFILAS) + getValor(A,i,k,ORDENXFILAS)*getValor(B,k,j,ORDENXCOLUMNAS));
      setValor(CA,i,j,ORDENXFILAS, getValor(CA,i,j,ORDENXFILAS) + getValor(C,i,k,ORDENXCOLUMNAS)*getValor(AT,k,j,ORDENXCOLUMNAS));
      setValor(BD,i,j,ORDENXFILAS, getValor(BD,i,j,ORDENXFILAS) + getValor(BT,i,k,ORDENXCOLUMNAS)*getValor(D,k,j,ORDENXCOLUMNAS));
    }
   }
  }   
